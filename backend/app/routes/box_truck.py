...
router = APIRouter(prefix="/box-trucks", tags=["Box Truck Services"])
...
# 2. GET BY SLUG
@router.get("/{slug}", response_model=BoxTruckResponse)
def get_box_truck_by_slug(slug: str, db: Session = Depends(get_db)):
    truck = db.query(BoxTruck).filter(BoxTruck.slug == slug).first()
    if not truck:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail="Box truck service card not found"
        )
    return truck
...
async def create_box_truck(
    ...
    if custom_id is not None:
        if db.query(BoxTruck).filter(BoxTruck.id == custom_id).first():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Card with ID {custom_id} already exists."
            )

    if db.query(BoxTruck).filter(BoxTruck.slug == slug).first():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
...
async def update_box_truck(
    ...
    if short_description is not None: truck.short_description = short_description
    if detail_heading is not None: truck.detail_heading = detail_heading
    
    if meta_title is not None: truck.meta_title = clean_string(meta_title)
    if meta_description is not None: truck.meta_description = clean_string(meta_description)
    if meta_keywords is not None: truck.meta_keywords = clean_string(meta_keywords)
    if canonical_url is not None: truck.canonical_url = clean_string(canonical_url)

    new_card_img = save_uploaded_file(card_image_file)
    if new_card_img:
        truck.card_image = new_card_img

    new_detail_img = save_uploaded_file(detail_image_file)
    if new_detail_img:
        truck.detail_image = new_detail_img

    if features is not None:
        truck.features = parse_to_list(features)
...
def delete_box_truck(id: int, db: Session = Depends(get_db)):
    ...
    db.delete(truck)
    db.commit()
    return {"success": True, "message": "Box truck card deleted successfully"}